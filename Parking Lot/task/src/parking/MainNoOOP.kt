...
fun main() {
    ...
    loop@ while (true) {
        when (scanner.next()) {
            ...
            "reg_by_color" -> searchByColor(spots, scanner, "regNum")
            "spot_by_color" -> searchByColor(spots, scanner, "spotNum")
            "spot_by_reg" -> {
                if (checkEmptyLot(spots)) continue@loop
...
private fun searchByColor(spots: Array<String>, scanner: Scanner, printData: String) {
